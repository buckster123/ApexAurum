...
# =============================================================================
# Resolve paths: pages/music_visualizer.py -> pages/ -> ApexAurum/
_THIS_DIR = Path(__file__).parent.resolve()
_PROJECT_ROOT = _THIS_DIR.parent  # pages/ -> ApexAurum

# Find sandbox in project structure
if (_PROJECT_ROOT / "sandbox").exists():
    SANDBOX_ROOT = _PROJECT_ROOT / "sandbox"
else:
    SANDBOX_ROOT = Path(st.session_state.get("sandbox_dir", "./sandbox")).resolve()

MUSIC_ROOT = SANDBOX_ROOT / "music"
PRESETS_DIR = SANDBOX_ROOT / "visualizer_presets"
...
class AudioAnalyzer:
    ...
    def _get_waveform_bands_at(self, time: float, config: EffectSettings) -> Dict[str, np.ndarray]:
        ...
        if preset == WaveformFreqPreset.BASS_ONLY:
            ...
            # Full custom mode
            splits = config.waveform_band_freq_splits
        ...
        if self.config.effects.enable_waveform:
            ...
            if self.config.effects.waveform_band_mode == WaveformBandMode.SINGLE:
                ...
                if self.config.effects.waveform_fill:
                    # Use Polygon patch for fill mode (supports set_xy)
                    poly = Polygon([[0, 0]], closed=True,
                                   facecolor=self.config.effects.waveform_band_colors[0],
                                   alpha=0.3, edgecolor=self.config.effects.waveform_band_colors[0],
                                   linewidth=0.5)
                    ax_waveform.add_patch(poly)
                    waveform_lines = [poly]
                else:
            ...
            if self.config.effects.waveform_band_mode == WaveformBandMode.SINGLE:
                ...
                if len(waveform) > 0:
                    ...
                    if wf_is_vertical:
                        ...
                        if self.config.effects.waveform_fill:
                            # Create closed polygon: waveform -> baseline -> back
                            x_verts = np.concatenate([waveform, np.zeros_like(waveform)[::-1]])
                            y_verts = np.concatenate([y_data, y_data[::-1]])
                            ax_dict['waveform_lines'][0].set_xy(np.column_stack([x_verts, y_verts]))
                        else:
                        ...
                        if self.config.effects.waveform_fill:
                            # Create closed polygon: waveform -> baseline -> back
                            x_verts = np.concatenate([x_data, x_data[::-1]])
                            y_verts = np.concatenate([waveform, np.zeros_like(waveform)[::-1]])
                            ax_dict['waveform_lines'][0].set_xy(np.column_stack([x_verts, y_verts]))
                        else:
                ...
                for i, (line, band_name) in enumerate(zip(ax_dict['waveform_lines'], band_names)):
                    if band_name in waveform_bands and len(waveform_bands[band_name]) > 0:
                        waveform = waveform_bands[band_name].copy()
                        ...
                        if wf_is_vertical:
                            ...
                            if self.config.effects.waveform_fill:
                                # Create closed polygon: waveform -> baseline -> back
                                baseline = np.zeros_like(waveform) if wf_render_mode != WaveformRenderMode.PARALLEL else np.full_like(waveform, offset if 'offset' in dir() else 0)
                                x_verts = np.concatenate([waveform, baseline[::-1]])
                                y_verts = np.concatenate([y_data, y_data[::-1]])
                                line.set_xy(np.column_stack([x_verts, y_verts]))
                            else:
                            ...
                            wf_offset = 0
                            if wf_render_mode == WaveformRenderMode.STACKED:
                                wf_offset = (i - (n_bands-1)/2) * 0.5
                                waveform = waveform + wf_offset
                            elif wf_render_mode == WaveformRenderMode.PARALLEL:
                            ...
                            if self.config.effects.waveform_fill:
                                # Create closed polygon: waveform -> baseline -> back
                                baseline = np.full_like(waveform, wf_offset)
                                x_verts = np.concatenate([x_data, x_data[::-1]])
                                y_verts = np.concatenate([waveform, baseline[::-1]])
                                line.set_xy(np.column_stack([x_verts, y_verts]))
                            else:
...
def music_visualizer_page():
    ...
    with tab1:
        ...
        # Load music_tasks.json for rich metadata
        music_tasks_path = SANDBOX_ROOT / "music_tasks.json"
        music_tasks = {}
        audio_to_task = {}  # Map audio filename to task data

        if music_tasks_path.exists():
            try:
                with open(music_tasks_path, 'r') as f:
                    music_tasks = json.load(f)
                # Build lookup: audio filename -> task info
                for task_id, task in music_tasks.items():
                    if task.get("audio_file"):
                        # Normalize path for matching
                        audio_filename = Path(task["audio_file"]).name
                        audio_to_task[audio_filename] = task
            except Exception as e:
                logger.warning(f"Could not load music_tasks.json: {e}")
        ...
        def format_audio_option(path: Path) -> str:
            """Format audio file with rich metadata from music_tasks.json"""
            filename = path.name
            task = audio_to_task.get(filename)

            if task:
                # Build rich display string
                parts = []
                ...
                if style:
                    style_short = style[:25] + "..." if len(style) > 25 else style
                    parts.append(f"[{style_short}]")
                ...
                # No task metadata - just show filename
                return filename

        selected_audio = st.selectbox(
            "Select Audio",
            options=audio_files,
            format_func=format_audio_option
        )

        st.divider()
        st.subheader("🖼️ Cover Image")

        # Auto-detect matching cover
        auto_cover_path = None
        for ext in ['.jpg', '.png', '.jpeg']:
            candidate = selected_audio.with_suffix(ext)
            if candidate.exists():
                auto_cover_path = candidate
                break

        # Cover source selection
        cover_source = st.radio(
            "Cover source",
            ["Auto-detect", "Upload custom"],
            horizontal=True,
            help="Auto-detect looks for image with same name as audio file"
        )

        cover_path = None

        if cover_source == "Auto-detect":
            if auto_cover_path:
                cover_path = auto_cover_path
                st.success(f"Found: {auto_cover_path.name}")
                st.image(str(auto_cover_path), width=300)
            else:
                st.warning("No matching cover found. Try uploading one!")
        else:
            # Custom upload
            uploaded_cover = st.file_uploader(
                "Upload cover image",
                type=["jpg", "jpeg", "png"],
                help="Square images work best (e.g., 1000x1000)"
            )

            if uploaded_cover:
                # Save uploaded file to temp location
                temp_cover_path = SANDBOX_ROOT / "visualizer_presets" / f"_temp_cover_{uploaded_cover.name}"
                with open(temp_cover_path, "wb") as f:
                    f.write(uploaded_cover.getbuffer())
                cover_path = temp_cover_path
                st.success(f"Using: {uploaded_cover.name}")
                st.image(uploaded_cover, width=300)
            else:
                st.info("Upload an image to use as cover")
    ...
    with tab2:
        ...
        with wf_col2:
            ...
            if st.session_state.viz_config.effects.enable_waveform:
                ...
                st.session_state.viz_config.effects.waveform_render_mode = WaveformRenderMode(
                    st.selectbox(
                        "Rendering Mode",
    ...
    with tab4:
        ...
        if not cover_path:
            st.error("⚠️ No cover image selected. Go to **Files** tab to auto-detect or upload one.")
            st.stop()

        if st.button("👁️ Generate 5s Preview"):
            preview_path = selected_audio.with_suffix('.preview.mp4')
            # Use deep copy so preview settings don't affect main config
            preview_config = copy.deepcopy(st.session_state.viz_config)
